...
Services own all business logic. Controllers are thin — they only handle HTTP concerns.
...
| `JWT_SECRET`  | (see yml)   | Base64 HMAC-SHA256 key |

---

## Seeded Credentials

Three users are created automatically by the Flyway migration:
...
- Dashboard: role-based access, summary data correctness
- Users: create with audit, role update with audit, RBAC enforcement
...
**Native SQL for trend aggregation** — The monthly trends query uses `DATE_TRUNC` and `TO_CHAR`, which are PostgreSQL-specific. This is the right tradeoff — trying to express a group-by-month aggregation in JPQL would produce inferior SQL. The test profile uses H2 to remain database-agnostic for unit tests.

**BCrypt cost factor 12** — Slightly slower than the default of 10, but still well within acceptable login latency (~300ms). This makes brute-force attacks significantly more expensive.
